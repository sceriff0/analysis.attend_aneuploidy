...
# the SAME pair as report 07 is deliberate: any difference between the two reports is then the
...
report 07's ATTEND clusters need a second look before they are interpreted.
...
* **The peaks are published.** Report 07 clusters peaks GISTIC learned *from ATTEND*, and a
...
pipeline's configured pair, exactly as report 07 does.
...
**TP53 is absent from this report's annotation stack**, unlike report 07's. `data/tcga_2013/`
...
Here it answers a question report 07 cannot — TCGA fixed k = 4 and published the result, so
...
heuristic has failed**, and since report 07 uses that same rule on ATTEND, where no published
