...
**Consumed by:** `analysis/09-tcga-classification.Rmd`.

This is an operational recipe, run once per cohort freeze, not per knit. The rationale for
using GISTIC thresholded peaks as the clustering feature is in `analysis/00-methods.Rmd`.


The copy-number clustering in `09-tcga-classification.Rmd` **requires GISTIC output in
...
**Step 3 — knit `analysis/09-tcga-classification.Rmd`.** `load_gistic_thresholded()`
