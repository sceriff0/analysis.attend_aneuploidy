...
#
# Prereqs: report 01 has written the master; data/seg/*.seg present;
...
if (need_groups || need_loo) {
  # --- master + scna_group (same composition report 06 uses) -----------------
  master <- tryCatch(read_intermediate("attend_master_joined"), error = function(e) NULL)
  if (is.null(master) || !nrow(master))
    stop("master table absent — knit report 01 first (output/clean_data/attend_master_joined).")
